...
public class HostTest {
    ...
    public void testHost() throws Exception {
        ...
        assertEquals(callCount, host.getTotalConnections());
        assertEquals(0, host.getTotalErrors());
        assertEquals(0, host.getConsecutiveErrors());
        assertEquals(0, host.getOpenConnections());
        assertEquals(0, host.getResponseIndex());

        // test open connections
        host.connectionOpened();
        host.connectionOpened();

        assertEquals(2, host.getOpenConnections());
        assertEquals(2, host.getResponseIndex());
        ...
        assertEquals(0, host.getOpenConnections());
        assertEquals(1, host.getConsecutiveErrors());
        assertEquals(1, host.getTotalErrors());
        assertEquals(0, host.getResponseIndex());
        assertFalse(host.isHealthy()); // host should enter cool down period for error

        Thread.sleep(errorWaitTime - 500); // wait until just before the error is cooled down
        assertFalse(host.isHealthy()); // host should still be in cool down period
        ...
        assertEquals(0, host.getOpenConnections());
        assertEquals(2, host.getConsecutiveErrors());
        assertEquals(2, host.getTotalErrors());
        assertEquals(0, host.getResponseIndex());
        ...
        assertEquals(0, host.getOpenConnections());
        assertEquals(3, host.getConsecutiveErrors());
        assertEquals(3, host.getTotalErrors());
        assertEquals(0, host.getResponseIndex());
        assertFalse(host.isHealthy());
        ...
        assertEquals(0, host.getConsecutiveErrors());
        assertEquals(3, host.getTotalErrors());
        assertEquals(callCount + 5, host.getTotalConnections());
        assertEquals(0, host.getResponseIndex());
        assertTrue(host.isHealthy());
    }
    ...
    public void testErrorWaitLimit() throws Exception {
        ...
        assertEquals(errors, host.getConsecutiveErrors());
