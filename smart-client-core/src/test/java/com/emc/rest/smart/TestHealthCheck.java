...
public class TestHealthCheck {
    private static final Logger log = LoggerFactory.getLogger(TestHealthCheck.class);
    ...
    public void testUnhealthyHostIgnored() {
        ...
        for (HostStats stats : loadBalancer.getHostStats()) {
            if (stats.equals(foo)) {
                assertEquals(0, stats.getTotalConnections(), "unhealthy host should be ignored");
            } else {
                assertTrue(Math.abs(callCount / (hostList.length - 1) - stats.getTotalConnections()) <= 3, "unbalanced call count");
            }
        }
    }
    ...
    public void testHealthyUpdate() throws Exception {
        ...
        assertEquals(4, loadBalancer.getAllHosts().size());
